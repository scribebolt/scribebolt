"use server"

import { createClient } from "@supabase/supabase-js"

/**
 * --------------------------------------------------------------------
 * SUPABASE  ─ create an admin-level client (service-role key required)
 * --------------------------------------------------------------------
 */
const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!)

/* ------------------------------------------------------------------ */
/*  Types                                                             */
/* ------------------------------------------------------------------ */
export interface AdminUser {
  id: string
  email: string
  first_name: string | null
  last_name: string | null
  company: string | null
  plan: string
  status: string
  created_at: string
  last_sign_in_at: string | null
  email_confirmed_at: string | null
  onboarding_completed: boolean
  role: string
}

/* ------------------------------------------------------------------ */
/*  Helpers                                                           */
/* ------------------------------------------------------------------ */
function emptyStats() {
  return { totalUsers: 0, activeUsers: 0, trialUsers: 0, suspendedUsers: 0 }
}

/* ------------------------------------------------------------------ */
/*  Public API                                                         */
/* ------------------------------------------------------------------ */

/**
 * Fetch all user rows (profiles ← auth.users join) for Admin table
 */
export async function fetchUsers(): Promise<AdminUser[]> {
  // get profile rows
  const { data: profiles, error } = await supabase
    .schema("public")
    .from("profiles")
    .select(
      `
      id,
      email,
      first_name,
      last_name,
      company,
      plan,
      status,
      created_at,
      onboarding_data:onboarding_data(role, completed_at)
    `,
    )
    .order("created_at", { ascending: false })

  if (error) {
    console.error("Error fetching profiles:", error)
    return []
  }

  // auth meta (last_sign_in_at, email_confirmed_at)
  const { data: authList, error: authErr } = await supabase.auth.admin.listUsers()
  if (authErr) console.error("Error fetching auth users:", authErr)

  const users: AdminUser[] =
    profiles?.map((p) => {
      const authUser = authList?.users.find((au) => au.id === p.id)
      const onboarding = Array.isArray(p.onboarding_data) ? p.onboarding_data[0] : p.onboarding_data

      return {
        id: p.id,
        email: p.email ?? authUser?.email ?? "",
        first_name: p.first_name,
        last_name: p.last_name,
        company: p.company,
        plan: p.plan ?? "free",
        status: p.status ?? "active",
        created_at: p.created_at,
        last_sign_in_at: authUser?.last_sign_in_at ?? null,
        email_confirmed_at: authUser?.email_confirmed_at ?? null,
        onboarding_completed: !!onboarding?.completed_at,
        role: onboarding?.role ?? "unknown",
      }
    }) ?? []

  return users
}

/**
 * Aggregate simple statistics for cards at top of Users page
 */
export async function getUserStats() {
  try {
    const { data, error } = await supabase.schema("public").from("profiles").select("status, plan")

    if (error) throw error
    if (!data) return emptyStats()

    return {
      totalUsers: data.length,
      activeUsers: data.filter((u) => u.status === "active").length,
      trialUsers: data.filter((u) => u.plan === "trial").length,
      suspendedUsers: data.filter((u) => u.status === "suspended").length,
    }
  } catch (err: any) {
    // relation missing
    if (err.code === "42P01") {
      console.warn("profiles table missing – returning empty stats")
      return emptyStats()
    }
    console.error("getUserStats failed:", err)
    return emptyStats()
  }
}

/**
 * Update a user’s status (active / suspended)
 */
export async function updateUserStatus(userId: string, status: string) {
  const { error } = await supabase.schema("public").from("profiles").update({ status }).eq("id", userId)

  return error ? { success: false, error: error.message } : { success: true }
}

/**
 * Permanently delete a user (auth.users + cascade profile row)
 */
export async function deleteUser(userId: string) {
  const { error } = await supabase.auth.admin.deleteUser(userId)
  return error ? { success: false, error: error.message } : { success: true }
}
